...
def communicate_server(addr, data, recvBufferSize=8192):
    retData = b""
...
def server_get_license_key(addr):
    ...
    server_response = communicate_server(addr, json.dumps(request).encode())
...
def server_get_license_info(addr, licenseKey):
    ...
    response = json.loads(
        communicate_server(addr, json.dumps(request).encode()).decode()
    )
    if "SUCCESS" in response.keys():
        if response["SUCCESS"]:
